...
<input type="email" name="email" style="margin-bottom:8px; width:384px" placeholder="Email" value="' . htmlspecialchars($user['email']) . '" required/><br>
<input type="text" name="team_name" style="margin-bottom:8px; width:384px" placeholder="Team name" minlength="' . Config::get('MIN_TEAM_NAME_LENGTH') . '" maxlength="',Config::get('MAX_TEAM_NAME_LENGTH'),'" value="' . htmlspecialchars($user['team_name']) . '" required/><br>';

$countries = api_get_countries();

echo '<select name="country" style="width: 384px; margin-bottom:8px" required>';
...
if ($user['2fa_status'] == 'generated') {
    ...
    echo '<input style="margin-bottom:8px; width:384px" type="text" name="code" placeholder="2FA Code" required/><br>';
